...
import { ArrowLeft, FilePlus2, Link2, Pencil, ReceiptText, WalletCards } from "lucide-react";
import { linkCustomerSalesByName } from "@/app/actions";
...
export default async function CustomerProfilePage({
  ...
  const { count: unlinkedMatchingSalesCount = 0 } = await supabase
    .from("sales")
    ...
    <div className="flex min-h-screen bg-slate-50/50">
      ...
      <main className="flex-1 overflow-y-auto p-4 pt-20 sm:p-6 sm:pt-24 lg:p-8 lg:pt-8">
        ...
        <PageActionStrip
          ...
          extra={
            <ConfirmActionForm
              action={linkCustomerSalesByName}
              confirmMessage={`Link ${unlinkedMatchingSalesCount ?? 0} old sales with customer name "${customer.name}" to this profile?`}
              hiddenFields={[{ name: "customer_id", value: customer.id }]}
            >
              <Button
                type="submit"
                variant="primary"
                size="lg"
                disabled={!unlinkedMatchingSalesCount}
                className="w-full shrink-0 whitespace-nowrap sm:w-auto md:min-w-fit"
              >
                <Link2 className="h-[var(--ui-icon-sm)] w-[var(--ui-icon-sm)]" />
                Link
                <span className="rounded-full bg-white/20 px-2 py-0.5 text-xs text-white">
                  {unlinkedMatchingSalesCount ?? 0}
                </span>
              </Button>
            </ConfirmActionForm>
          }
